...
$delete= mysqli_query($conn,"DROP TABLE data3");
// die();
...
if($header){
    ...
    $sql[] = 'CREATE TABLE `data3` ('.implode(',',$header_sql).')';
    // $sql[] = 'ALTER TABLE `data2` ADD `Mahindra` VARCHAR(255)';
    while($data = fgetcsv($handle,1000,",")){   
        $sql[] = "INSERT INTO data3 VALUES ('".implode("','",$data)."')";
    }
     $sql[] = "ALTER TABLE `data3` ADD $title VARCHAR(255)";
}        
foreach($sql as $s){
  if($s!=null)
    mysqli_query($conn,$s);
