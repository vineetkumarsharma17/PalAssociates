...
//$delete= mysqli_query($conn,"DROP TABLE data2");
// die();
...
if($header){
    ...
    $sql[] = 'CREATE TABLE `data2` ('.implode(',',$header_sql).')';
    // $sql[] = 'ALTER TABLE `data2` ADD `Mahindra` VARCHAR(255)';
    while($data = fgetcsv($handle,1000,",")){   
        $sql[] = "INSERT INTO data2 VALUES ('".implode("','",$data)."')";
    }
     $sql[] = "ALTER TABLE `data2` ADD $title VARCHAR(255)";
}        
foreach($sql as $s){
  if($s!=null)
    mysqli_query($conn,$s);
